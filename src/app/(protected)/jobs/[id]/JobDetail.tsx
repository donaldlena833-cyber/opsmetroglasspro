...
export function JobDetail({ job: initialJob }: JobDetailProps) {
  ...
  const handleDeleteExpense = async () => {
    ...
    setJob({
      ...job,
      expenses: job.expenses?.filter(e => e.id !== deleteExpenseId) || []
    })
  ...
  const handleDeletePayment = async () => {
    ...
    setJob({
      ...job,
      payments: job.payments?.filter(p => p.id !== deletePaymentId) || []
    })
    toast({ title: 'Deleted', description: 'Payment removed', variant: 'success' })
          ...
          {(!job.expenses || job.expenses.length === 0) ? (
            ...
            <div className="space-y-2">
              {job.expenses.map((expense) => {
                const categoryConfig = expenseCategoryConfig[expense.category as ExpenseCategory]
                return (
          ...
          {(!job.payments || job.payments.length === 0) ? (
            ...
            <div className="space-y-2">
              {job.payments.map((payment: any) => {
                ...
                return (
                  <Card key={payment.id} className="p-4 relative group">
                    <div className="flex items-start justify-between">
                      <div>
      ...
      <Dialog open={!!deleteExpenseId} onOpenChange={() => setDeleteExpenseId(null)}>
        <DialogContent>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button variant="destructive" onClick={handleDeleteExpense} loading={deletingItem}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!deletePaymentId} onOpenChange={() => setDeletePaymentId(null)}>
        <DialogContent>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button variant="destructive" onClick={handleDeletePayment} loading={deletingItem}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
